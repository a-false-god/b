...
export function BottomTabBar({ currentTab, onTabChange }: BottomTabBarProps) {
  ...
  return (
    <nav
      ...
      className="fixed bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-1.5rem)] max-w-[480px] z-50 rounded-[16px] bg-card/95 backdrop-blur-md tab-bar-shadow border border-border px-1 py-1 flex items-center justify-between transition-all"
    >
      {tabs.map((tab) => {
        const Icon = tab.icon
        ...
        return (
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center py-1.5 px-1 rounded-[10px] transition-all duration-150 select-none ${
              isActive
